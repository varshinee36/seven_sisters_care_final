...
/// Lightweight Q-Learning Reinforcement Learning (RL) Adaptive Engine
/// for Cognitive Games in Seven Sisters Care.
class CognitiveAdaptiveEngine {
  static final CognitiveAdaptiveEngine instance =
  ...
  static const double _timerStepSeconds =
      3.0; // Gradual timer adjustment step
  ...
  static final ValueNotifier<List<GamePerformanceRecord>>
  ...
  double _initialQValue(
    ...
    switch (state) {
      case PerformanceState.highPerformance:
        ...
        if (action == AdaptiveAction.increaseDifficultyMaintainTimer) {
          return 0.4;
        }
        if (action == AdaptiveAction.maintainDifficultyMaintainTimer) {
          return 0.1;
        }
        return -0.2;
      ...
      case PerformanceState.challenging:
        if (action == AdaptiveAction.maintainDifficultyIncreaseTimer) {
          return 0.5;
        }
        if (action == AdaptiveAction.maintainDifficultyMaintainTimer) {
          return 0.3;
        }
        if (action == AdaptiveAction.decreaseDifficultyIncreaseTimer) {
          return 0.2;
        }
        return -0.3;
  ...
  static const String _keyLastPlayedGame =
      'cognitive_last_played_game_id';

  /// Saves the final game performance record for a particular game.
  Future<void> saveGamePerformanceRecord(
    GamePerformanceRecord record,
  ) async {
    try {
      final prefs = await SharedPreferences.getInstance();

      await prefs.setString(
        '$_prefixGamePerf${record.gameId}',
        record.encode(),
      );
      ...
      final current =
          List<GamePerformanceRecord>.from(
        latestPerformanceRecords.value,
      );
      ...
      debugPrint(
        'Error saving game performance record: $e',
      );
    }
  }

  /// Retrieves the stored final performance record for a particular game.
  Future<GamePerformanceRecord?> getGamePerformanceRecord(
    String gameId,
  ) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      ...
      debugPrint(
        'Error retrieving game performance record for $gameId: $e',
      );
    }
  ...
  Future<GamePerformanceRecord?> getLastPlayedGameRecord() async {
    try {
      ...
      final lastGameId = prefs.getString(
        _keyLastPlayedGame,
      );
      ...
      debugPrint(
        'Error retrieving last played game record: $e',
      );
    }
  ...
  /// - Return to SAME game: starting timer = stored lastTimerValue
  ///   (never reset to 60s!), entry level mapped from previous performance.
  /// - Entry into a DIFFERENT game: starting level determined
  ///   GRADUALLY & CONSERVATIVELY from previous game score
  ///   (max level 2 or 3). Starting timer = target game's stored
  ///   timer or 60s if new.
  Future<({int startingLevel, double startingTimer})>
      determineEntryLevelAndTimer({
    required String targetGameId,
    int maxLevels = 7,
  }) async {
    final targetRecord =
        await getGamePerformanceRecord(targetGameId);

    final lastPlayedRecord =
        await getLastPlayedGameRecord();

    // 1. SAME GAME RETURN:
    // Player has played this particular game before
    if (targetRecord != null) {
      final prevScore =
          targetRecord.finalPerformanceScore;

      final savedTimer =
          targetRecord.lastTimerValue > 0
              ? targetRecord.lastTimerValue
              : 60.0;
      ...
      entryLevel =
          entryLevel.clamp(1, maxLevels);

      return (
        startingLevel: entryLevel,
        startingTimer:
            savedTimer.clamp(15.0, 60.0),
      );
    }

    // 2. DIFFERENT GAME ENTRY:
    // Player hasn't played target game, but played a previous game
    if (lastPlayedRecord != null &&
        lastPlayedRecord.gameId != targetGameId) {
      final prevScore =
          lastPlayedRecord.finalPerformanceScore;

      // Conservative starting-level mapping for
      // elderly dementia patients
      // (never jump to level 4 or 5!)
      ...
      conservativeLevel =
          conservativeLevel.clamp(1, maxLevels);

      // Target game is new -> initial timer = 60.0s
      return (
        startingLevel: conservativeLevel,
        startingTimer: 60.0,
      );
    }
  ...
  AdaptiveAction selectAction(
    ...
    var bestAction =
        AdaptiveAction.maintainDifficultyMaintainTimer;
  ...
  /// Internal RL Reward signal calculation
  /// (NOT shown to patient).
  double computeReward(
    LevelPerformanceMetrics metrics,
    AdaptiveAction action,
  ) {
  ...
  void updateQValues(
    ...
    final nextMaxQ =
        _qTable[nextState]!.values.reduce(max);
  ...
  ({
    ...
    session.cumulativePerformance
        .addLevelMetrics(levelMetrics);

    // 2. Select action via Q-Learning
    AdaptiveAction action =
        selectAction(currentState);

    // Override action for specific edge cases
    // (e.g. timeout or near-timeout)
    if (levelMetrics.isTimeout) {
      action =
          AdaptiveAction.decreaseDifficultyIncreaseTimer;
    } else if (levelMetrics.timeUtilization >= 0.90) {
      // Finished near the end of timer ->
      // gradually increase timer
      if (action ==
          AdaptiveAction.increaseDifficultyReduceTimer) {
        action =
            AdaptiveAction.increaseDifficultyMaintainTimer;
      } else if (action ==
          AdaptiveAction.maintainDifficultyMaintainTimer) {
        action =
            AdaptiveAction.maintainDifficultyIncreaseTimer;
      }
    }

    // 3. Compute reward and update Q-table
    final reward =
        computeReward(levelMetrics, action);
    ...
    switch (action) {
      case AdaptiveAction.increaseDifficultyReduceTimer:
        nextLevel =
            min(maxLevels, session.currentLevel + 1);
    ...
    // On successful level completion, ensure progression
    // always advances to higher level
    if (levelMetrics.isSuccess &&
        session.currentLevel < maxLevels) {
      nextLevel = min(
        maxLevels,
        max(
  ...
  /// saves particular-game record locally, and dispatches to MongoDB
  /// via AnalyticsService.
  Future<GamePerformanceRecord> endSession(
    AdaptiveSessionState session,
  ) async {
    final finalScore =
        session.cumulativePerformance.cumulativeScore
            .clamp(0.0, 100.0);
    ...
    final mongoPayload = {
      ...
      'cumulative_accuracy':
          session.cumulativePerformance.cumulativeAccuracy,
      'cumulative_correct_answers':
          session.cumulativePerformance.totalCorrectAnswers,
      'cumulative_wrong_answers':
          session.cumulativePerformance.totalWrongAnswers,
      'average_completion_time':
          session.cumulativePerformance.averageCompletionTime,
    ...
    try {
      ...
      debugPrint(
        'Error recording game performance to MongoDB: $e',
      );
    }
  ...
  String _formatPerformanceState(
    PerformanceState state,
  ) {
    switch (state) {
      case PerformanceState.highPerformance:
